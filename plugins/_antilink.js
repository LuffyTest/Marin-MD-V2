...
handler.before = async function (m, { isAdmin, isBotAdmin }) {
  ...
  if (chat.antiLink && ValidLink && !isAdmin && !m.isBaileys && m.isGroup) {
    ...
    await conn.send2But(m.chat, `*⚠️ Link Detected! ⚠️*${isBotAdmin ? '' : '\n\nI\'m not an admin so I can\'t kick t_t'}\n\nType *.off antilink* to turn off this feature${opts['restrict'] ? '' : '\ntype *.on restrict* so you can kick'}`, wm, 'Turn off Antilink', '.off antilink', 'Turn on restrict', '.on restrict', m)
